...
def deterministic_run(precision,initial_contamination,r,C,m,g,l):
    ...
    return([num_at_0,num_at_1,num_at_2,num_at_4,num_at_8,num_at_12,num_at_24,num_at_48])


Detergent_Means=[[np.zeros((1,8)) for i in range(3)] for j in range(4)]
Detergent_SD=[[np.zeros((1,8)) for i in range(3)] for j in range(4)]

Disinfectant_Means=[[np.zeros((1,8)) for i in range(3)] for j in range(4)]
Disinfectant_SD=[[np.zeros((1,8)) for i in range(3)] for j in range(4)]
...
Detergent_Means[0][0] = [9.15,6.3,5.833333333,5.075,6.075,5.25,7.475,7.9]
Detergent_SD[0][0] = [6.962832361,6.080069192,4.799125878,4.960685958,4.676219185,2.876779809,3.814869953,5.247166723]
...
Detergent_Means[1][0] = [5.883333333,6.266666667,4.658333333,2.833333333,2.966666667,4.858333333,4.841666667,2.966666667]
Detergent_SD[1][0] = [7.981958824,8.210562303,6.438437095,5.271523341,4.197494292,6.326366782,7.91168703,5.682868994]
...
# File with posterior
Posterior = open("Posterior.txt","w")
...
while len(parameter_sample) < sample_size:
	...
	trial_C = random.uniform(2.0,12.0)
