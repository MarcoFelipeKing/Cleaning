...
initial_contamination=4.14 #59
measurement_data = np.array([2.10,2.14,2.25,2.63,4.49])#np.array([19,5,5,2,9])
s=np.array([26,2.3,4.67,4.33,4.27])
precision=5000


measurement_times = np.array([1,2,4,8,24,48])#np.arange(len(measurement_data))  
...
def ode_model(contamination,t,r,C,d,g,l):
    Contamination = contamination;
    return(Contamination*r*(1-Contamination/C)-d*math.exp(-g*t)*Contamination+l)
...
def deterministic_run(parameters):#precision,initial_contamination,r,C,d,g):
    ...
    tmax = 24
    time_space = np.linspace(0,tmax,precision+1)
    sim=odeint(ode_model,initial_contamination,time_space,args=(parameters["r"],parameters["C"],parameters["d"],parameters["g"],parameters["l"]))
    #num_at_0=sim[int(precision*0.1/50.0)]
    num_at_1=sim[int(precision*1/50.0)]
    num_at_2=sim[int(precision*2/50.0)]
    num_at_4=sim[int(precision*4/50.0)]
...
pyabc.visualization.plot_histogram_1d(history_acceptor,x="l")
